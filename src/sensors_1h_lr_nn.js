...
var maWindow = 30; //size of moving average window in minutes
...
var NN = analytics.newNN({"layout": [FSSolar.dim /*+ FSWthrATVISCC.dim*/ + nAreg * 0, 6, 1], "tFuncHidden":"tanHyper", "tFuncOut":"linear", "learnRate":0.1, "momentum":0.6});

var rememberDate = "1970-09-07T12:12:12";
var outFile = fs.openWrite("sandbox/sensors/output.txt");
outFile.writeLine(
    "from: \tto: \tmax: \tmin: \tavg: \tcount: \tavgErrAll: \tavgErrSW: \tavgErrSWAreg: \tavgErrSWFcast: \tavgErrSWFcastIcept: \tavgErrSWFcastNN: \tavgErrAllSq: \tavgErrSWSq: \tavgErrSWAregSq: \tavgErrSWFcastSq: \tavgErrSWFcastIceptSq: \tavgErrSWFcastNNSq: "
);
        ...
        */
        ...
        if(datePredictingFor < sunriseTime || dateNow > sunsetTime){ 
            ...
            if(maWindow == 60)
                ...
                var WFcast = qm.search({ "$from" : "WPredHour" , "DTString" : WFuture.DateTime.string.replaceAt(14, "0").substring(0, WFuture.DateTime.string.length - 2 )});
            // prediction using NNETWORKS with weather prediction from forecast.io and solar angles -------------------------------------------------------------------------------
            // setup with prevoius values
            var ftrVSolWFcastNN = normalize(FSSolar.ftrVec(val), [90, 180]);
            /*ftrVSolWFcastNN.pushV(oldValsCVec);
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[2].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[3].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[4].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[5].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[6].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[7].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[8].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[9].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[10].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[11].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[12].toFixed(2));
            console.log("Preds: oldValsCVec: " + ftrVSolWFcastNN[13].toFixed(2));
            if(val.$id > nAreg){
                for(var n = 1; n <= nAreg; n++ ){
                    ftrVSolWFcastNN.pushV(normalize(FSWthrSR.ftrVec(WMeasAvg[val.$id - n]), 1000));
                }                
                for(var n = 1; n <= nAreg; n++ ){
                    ftrVSolWFcastNN.pushV(normalize(FSWthrRel.ftrVec(WMeasAvg[val.$id - n]), 150));
                }                
            }
            else{
                var tempVec = Array.apply(null, new Array(nAreg*2)).map(Number.prototype.valueOf,0);
                ftrVSolWFcastNN.pushV(linalg.newVec(tempVec));
            }
            *//*
            ftrVSolWFcastNN.pushV(normalize(FSWthrATVISCC.ftrVec(WFcast[0]), [50, 17, 1]));
            console.log("Preds: Data: " + WFcast[0].temperature + " " + WFcast[0].visibility + " " + WFcast[0].cloud_cover + " " + WFcast[0].DateTime.string);
            console.log("Preds: Data: " + ftrVSolWFcastNN[0].toFixed(2) + " " + ftrVSolWFcastNN[1].toFixed(2) + " " + ftrVSolWFcastNN[2].toFixed(2)  + " " + ftrVSolWFcastNN[3].toFixed(2)  + " " + ftrVSolWFcastNN[4].toFixed(2));
            /*
            for(var n = 0; n < ftrVSolWFcastNN.length; n++){
                console.log("Item: " + n + " before " + ftrVSolWFcastNN[n])
            }
            */
            //normalisation done here by hand....
            //ftrVSolWFcastNN = normalize(ftrVSolWFcastNN);
            /*
            for(var n = 0; n < ftrVSolWFcastNN.length; n++){
            ...
            var nnpred = NN.predict(ftrVSolWFcastNN)[0] * 12;
            // maybe learn NNs even in the night
            ...
            if (trainRecId > 1) {
                ...
                if(maWindow == 60)
                    ...
                    var WFcast = qm.search({ "$from" : "WPredHour" , "DTString" : WMeasAvg[val.$id].DateTime.string.replaceAt(14, "0").substring(0, WMeasAvg[val.$id].DateTime.string.length - 2 )});

                var ftrVSolWFcastCpy = normalize(FSSolar.ftrVec(SMeasAvg[trainRecId]), [90, 180])
                /*ftrVSolWFcastCpy.pushV(normalize(oldValsCVec, 12));
                if(val.$id > nAreg){
                    for(var n = 1; n <= nAreg; n++ ){
                        ftrVSolWFcastCpy.pushV(normalize(FSWthrSR.ftrVec(WMeasAvg[val.$id - n]), 1000));
                    }                
                    for(var n = 1; n <= nAreg; n++ ){
                        ftrVSolWFcastCpy.pushV(normalize(FSWthrRel.ftrVec(WMeasAvg[val.$id - n]), 150));
                    }                
                }
                else{
                    var tempVec = Array.apply(null, new Array(nAreg*2)).map(Number.prototype.valueOf,0);
                    ftrVSolWFcastCpy.pushV(linalg.newVec(tempVec));
                }*//*
                ftrVSolWFcastCpy.pushV(normalize(FSWthrATVISCC.ftrVec(WFcast[0]), [50, 17, 1]));
                
                for(var n = 0; n < ftrVSolWFcastCpy.length; n++){
                    //ftrVSolWFcastNN[n] = ftrVSolWFcastNN[n]/200;
                    console.log("Item: " + n + " " + ftrVSolWFcastCpy[n])
                }
                console.log("Item: ++++++++++++++++++++++++++++++++++");
                console.log("Item: TIME: " + val.DateTime.string)
                console.log("Item: WFCAST " + WFcast[0].DateTime.string)
                console.log("Preds: Learn from: " + SMeasAvg[trainRecId].DateTime.string)
                console.log("Preds: ooooooooooooooooooooooooooooooooooooooooooooooooooooo");
                var targetV = linalg.newVec([val.current_val / 12])
                NN.learn(ftrVSolWFcastCpy, targetV)
            }*/
            ...
            SMeasAvg.add({ 
                ...
                crnt_nn_solar_fcastio: val.current_val
            });
            ...
            for(var i = 0; i < predictBuffer.length; i++){
                for(var j = 0; j < predictBuffer[i].length; j++){
                    predictBuffer[i][j] = 0;
                }
            }
...
/*
            ...
            ftrVAll.pushV(FSWthrAll.ftrVec(WNow));
            ftrVAll.pushV(FSWthrRel.ftrVec(WFuture)); //Current weather data
            ...
            ftrVSolWPred.pushV(FSWthrRel.ftrVec(WFuture));
            ...
            ftrVSolWPred.pushV(oldValsCVec);
            ...
            ftrVSolWFcast.pushV(FSWthrFcast.ftrVec(WFcast[0]));
            ...
            */
            //ftrVSolWFcastNN.pushV(normalize(FSWthrATVISCC.ftrVec(WFcast[0]), [50, 17, 1]));
            //console.log("Preds: Data: " + WFcast[0].temperature + " " + WFcast[0].visibility + " " + WFcast[0].cloud_cover + " " + WFcast[0].DateTime.string);
            console.log("Preds: DataPred: " + FSSolar.ftrVec(val)[0].toFixed(2) + " " + FSSolar.ftrVec(val)[1].toFixed(2)) // + " " + ftrVSolWFcastNN[2].toFixed(2)  + " " + ftrVSolWFcastNN[3].toFixed(2)  + " " + ftrVSolWFcastNN[4].toFixed(2));
            /*
            for(var n = 0; n < ftrVSolWFcastNN.length; n++){
                console.log("Item: " + n + " before " + ftrVSolWFcastNN[n])
            }
            */
            ...
            preds.push(NN.predict(ftrVSolWFcastNN)[0]);
            /*
            for(var n = 0; n < preds.length; n++){
                //ftrVSolWFcastNN[n] = ftrVSolWFcastNN[n]/200;
                console.log("Preds: " + preds[n])
            }
            console.log("Preds: WEEK: " + countWeeks);
            */
            console.log("Preds: PREDICTED: " + preds[5])
            console.log("Preds: DateTime: " + val.DateTime.string);
            console.log("Preds: PredFor: " + datePredictingFor.toString());
            console.log("Preds: WEEK: " + countWeeks);
            ...
            if(predictBuffer.length > buffSize){
                ...
                console.say("DATE: crnt_nn_solar_fcastio " + predictBuffer[0][5])
                SMeasAvg.add({ 
                    $id: val.$id, 
                    ...
                    crnt_nn_solar_fcastio: predictBuffer[0][5]
                });
                predictBuffer.shift();
            }
            ...
            if (trainRecId > 1) {
                ...
                ftrVAll.pushV(FSWthrAll.ftrVec(WMeasAvg[trainRecId])) 
                ftrVAll.pushV(FSWthrRel.ftrVec(WMeasAvg[val.$id]));
                ...
                ftrVSolWPred.pushV(FSWthrRel.ftrVec(WMeasAvg[val.$id]));
                ...
                var ftrVSolWFcastCpy = normalize(FSSolar.ftrVec(SMeasAvg[trainRecId]), [90, 180])
                console.log("Preds: DataLrn: " + FSSolar.ftrVec(SMeasAvg[trainRecId])[0].toFixed(2) + " " + FSSolar.ftrVec(SMeasAvg[trainRecId])[1].toFixed(2)) // + " " + ftrVSolWFcastNN[2].toFixed(2)  + " " + ftrVSolWFcastNN[3].toFixed(2)  + " " + ftrVSolWFcastNN[4].toFixed(2));
                console.log("Preds: DataTrg: " + val.current_val.toFixed(2)) // + " " + ftrVSolWFcastNN[2].toFixed(2)  + " " + ftrVSolWFcastNN[3].toFixed(2)  + " " + ftrVSolWFcastNN[4].toFixed(2));
                /*ftrVSolWFcastCpy.pushV(normalize(oldValsCVec, 12));
                if(val.$id > nAreg){
                    for(var n = 1; n <= nAreg; n++ ){
                        ftrVSolWFcastCpy.pushV(normalize(FSWthrSR.ftrVec(WMeasAvg[val.$id - n]), 1000));
                ...
                console.log("Item: ++++++++++++++++++++++++++++++++++");
                console.log("Item: TIME: " + val.DateTime.string)
                console.log("Preds: WFCAST " + WFcast[0].DateTime.string)
                console.log("Preds: Learn from: " + SMeasAvg[trainRecId].DateTime.string)
                console.log("Preds: ----------------------------------");
                var targetV = linalg.newVec([val.current_val])
                NN.learn(ftrVSolWFcastCpy, targetV)
            }
            // check predictions
            var diff = [];
            ...
            if(Date.parse(val.DateTime.string) - Date.parse(rememberDate) > 1000*60*60*24*7){
                ...
                if(countWeeks > 23){
                    ...
                    outFile.writeLine(
                        ...
                        avgSumsAll[2].toFixed(2) + "\t" + 
                        avgSumsAll[3].toFixed(2) + "\t" + 
                        avgSumsAll[4].toFixed(2) + "\t" + 
                        avgSumsAll[5].toFixed(2) + " | \t" 
                    );                
                }
            }
        }
...
console.start()
*/
function normalize(ftrVec, divideBy){
