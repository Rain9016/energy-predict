...
qm.load.jsonFile(WPredARSHour, "sandbox/sensors/arsodata.json");
...
var NN = analytics.newNN({"layout": [FSSolar.dim + FSWthrARS.dim + nAreg * 3, 20, 1], "tFuncHidden":"tanHyper", "tFuncOut":"linear", "learnRate":0.1, "momentum":0.6});
...
SMeasAvg.addTrigger({
    onAdd: function (val) {
        ...
        */
        ...
        if(false/*datePredictingFor < sunriseTime || dateNow > sunsetTime*/){ 
...
/*
            ...
            ftrVSolWFcastNN.pushV(normalize(FSWthrARS.ftrVec(WFcast[0]), [1000, 50, 100]));
            //console.log("Preds: Data: " + WFcast[0].temperature + " " + WFcast[0].visibility + " " + WFcast[0].cloud_cover + " " + WFcast[0].DateTime.string);
            console.log("Preds: DataPred: " + ftrVSolWFcastNN[0].toFixed(2) + " " + FSSolar.ftrVec(val)[1].toFixed(2)) // + " " + ftrVSolWFcastNN[2].toFixed(2)  + " " + ftrVSolWFcastNN[3].toFixed(2)  + " " + ftrVSolWFcastNN[4].toFixed(2));
            ...
            if(predictBuffer.length > buffSize){
                SMeasAvg.add({ 
                    $id: val.$id, 
                    crnt_pred_all: 0, 
                    crnt_nn_solar_fcastio: /*preds[0]*/predictBuffer[0][0]
                });
                predictBuffer.shift();
            }
            ...
            console.log("Preds: CURR: " + val.DateTime.string ) 
            console.log("Preds: LAST: " + SMeasAvg[trainRecId].DateTime.string ) 
            // Learning part
            if (trainRecId > 1) {
                trainRecId--;
                // find hourly predictions even if interval less than 1 hour
                if(maWindow == 60)
                    var WFcast = qm.search({ "$from" : "WPredARSHour" , "DTString" : WMeasAvg[val.$id].DateTime.string.substring(0, WMeasAvg[val.$id].DateTime.string.length - 2 )});
                else
                    var WFcast = qm.search({ "$from" : "WPredARSHour" , "DTString" : WMeasAvg[val.$id].DateTime.string.replaceAt(14, "0").substring(0, WMeasAvg[val.$id].DateTime.string.length - 2 )});
                ...
                ftrVSolWFcastCpy.pushV(normalize(endBuffValsCVec, 12));
                if(val.$id > nAreg){
                    for(var n = 1; n <= nAreg; n++ ){
                ...
                ftrVSolWFcastCpy.pushV(normalize(FSWthrARS.ftrVec(WFcast[0]), [1000, 50, 100]));
